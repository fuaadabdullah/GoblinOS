...
const mockSecretKey = 'Vn25s4tnrbRBtOUxpCjvdSdCIkRcOHIAsS+4i0+YuI/9lyAJM4ED7L/M9/Dw7t3hYHrVT1uM9NlpH9jaR/CUvQ==';
...
describe('auditClient', () => {
  ...
  test('signs audit event correctly', async () => {
    ...
    const result = await sendSignedAudit(envelopeFields, { auditUrl: 'http://mock' });
  ...
  test('tamper detection - modified payload fails verification', async () => {
    ...
    const isValid = nacl.sign.detached.verify(tamperedBytes, signature, publicKey);
    assert(!isValid);
  });
});
