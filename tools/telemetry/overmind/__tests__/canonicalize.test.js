...
describe('canonicalize', () => {
  ...
  test('handles arrays with objects', () => {
    ...
    assert.strictEqual(canonicalize(obj), '{"arr":[{"a":2,"b":1},{"c":4,"d":3}]}');
  });
  ...
  test('matches Python json.dumps with sort_keys=True, separators=(",", ":"), ensure_ascii=False', () => {
    ...
    const obj = { "action": "test", "actor": "user", "context": { "note": "test", "trace_id": "123" } };
    const expected = '{"action":"test","actor":"user","context":{"note":"test","trace_id":"123"}}';
