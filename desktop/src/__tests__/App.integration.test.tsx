...
vi.mock("../api/tauri-client", () => {
  return {
    runtimeClient: {
      getGoblins: async () => [
        { id: "codesmith", name: "CodeSmith", title: "Coder", status: "active", brain: { router: "openai" } },
      ],
      getProviders: async () => ["ollama", "openai"],
      getCostSummary: async () => ({ total_cost: 0, cost_by_provider: {}, cost_by_model: {} }),
      executeTask: async (g: string, t: string) => ({ goblin: g, task: t, reasoning: "done", duration_ms: 10 }),
      executeTaskStreaming: async (g: string, t: string, onChunk: (c: string) => void, onComplete?: any) => {
        // simulate streaming tokens
        onChunk("hello ");
        onChunk("world");
        if (onComplete) onComplete({ reasoning: "hello world" });
      },
      parseOrchestration: async (text: string) => ({ steps: [], total_batches: 0, max_parallel: 0 }),
    },
  };
});
...
describe("runtimeClient smoke (mocked)", () => {
  it("returns goblins with brain.router and providers", async () => {
    ...
    expect(goblins[0].brain?.router).toBe("openai");
